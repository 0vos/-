    ...
    if (fileType.startsWith('text')) {
        ...
        reader.onloadend = function() {
            ...
            .then(data => {
                ...
                document.getElementById('file-decode').innerHTML = `<h3>编码内容: ${encodedContent}</h3>`;
        ...
        reader.onloadend = function() {
            ...
            .then(response => response.json())
            .then(data => {
                document.getElementById('file-decode').innerText = "上传成功！" + data.encoded;
            })
            .catch(error => {
                console.error('Error:', error);
                document.getElementById('file-decode').innerText = "上传失败：" + error.message;
    ...
    if (fileType.startsWith('text')) {
        ...
        reader.onloadend = function() {
            ...
            .then(data => {
                ...
                `<h3>文件内容</h3>
                <textarea type="text" rows="10" cols="50">${originContent}</textarea>
                <br>
                <button onclick="updateFile()">更新文件</button>
                <button onclick="searchString()">查找字符串</button>
